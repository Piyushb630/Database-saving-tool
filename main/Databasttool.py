...
excel_path = r'\\SFCG023A\P-55530$\FXMM and CLS Operations\Scripts (Work in Progress)\CRM FX GIVE UP\CRM_DATABASE.xlsx'
 
Organization_email = "crmprocessmanagement@seb.se"
...
def generate_salesapprovalemail():
    ...
    email.To = "fxcreditlink@seb.se"
    email.Body = f"Hello team,\n\nWe recently updated the FX GU onboarding process which form now on will require an approval from a Client Executive with relevant segment and sales before our team initiate onboarding process.\n\nPlease be advised that we have received a request to onboard {client_name} with {pb_name} as PB. Please find the Designation notice attached. Could you please advise if we can proceed with onboarding?"
    email.Display(False)
...
def generate_onboardingemail():
    ...
    email.SentOnBehalfOfName = Organization_email
    email.CC = Organization_email
    email.To = "securitiesfinanceam@seb.se"
    email.Body = f"Hello team,\n\nPlease be advised that we have received DN to onboard {client_name} with {pb_name}.\n\nWe have received the approval from Sales and Client executives as well.\n\nCould you please initiate the onboarding process."
    email.Display(False)
...
def generate_limitsemail():
    ...
    email.SentOnBehalfOfName = Organization_email
    email.CC = Organization_email
    email.To = "FICoverageBanksCAN@seb.se"
...
def generate_agreementsemail():
    ...
    email.SentOnBehalfOfName = Organization_email
    email.CC = Organization_email
    email.To = "marketsagreementsmanagement@seb.se"
    email.Body = (
        f"Hello team,\n\n"
        f"Please include GCN: {pb_name}/{client_name}: {gcn} as an IC to the ISDA and CSA for GCN {pb_name}: {value_d}.\n\n"
...
def terminate():
    ...
    email.Body = (
        f"Hello team,\n\n"
        f"@FICoverageBanksCAN@seb.se - Please remove the limits for {pb_name}/{client_name} {gcn} according to the attached EDN.\n\n"
        f"@marketsagreementsmanagement@seb.se - Please remove GCN: {pb_name}/{client_name}: {gcn} as an IC to the ISDA and CSA for GCN {pb_name}: {value_d}.\n\n"
        f"Please confirm once processed.\n"
    )
    email.Display(False)  # Display the email before sending
...
pb_names = ["Bank of America NA", "Barclays Bank Plc", "BNP Paribas", "Citibank", "Commerzbank", "Credit Suisse AG Zürich",
            "Credit Suisse International", "CSFB Credit Suisse AG, London Branch)", "Danske Bank", "Deutsche Bank AG", "Goldman Sachs",
            "HSBC", "ING Belgium", "Jefferies", "JPMorgan Chase Bank","JPMorgan Chase Bank LONDON" "Merrill Lynch International", "Morgan Stanley",
            "NatWestMarketsPlc(RBS)", "SocieteGeneraleInternationalLimited", "Standard Chartered Bank", "UBS AG"]
